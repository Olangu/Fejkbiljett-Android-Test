...
public class OrebroTicketTest extends TestCase {
	...
	protected void setUp() throws Exception {
		...
		for (int i=0; i<okTickets.length; i++) {
			...
			hmTicket.put(ticket, new OrebroTicket());
	...
	public void testGetMessage() {
		...
		for(int i=0;i<okTickets.length;i++)
		{
			...
			expectedString = message[5].substring(message[5].length()-3) + " LTÖ";
			assertEquals(expectedString,message[0]);
			
			assertEquals("", message[1]);
			
			expectedString = data.getBoolean("price_reduced")?"ÖS SKOLUNGDOM ":"ÖV VUXEN ";
			expectedString += "Giltig till " + validTime + " " + validDate;
			assertEquals(expectedString, message[2]);
			
			assertEquals("Örebro",message[3]);
	...
	public void testGetMessageOut() {
		...
		for(int i=0;i<okTickets.length;i++)
		{
			...
			expectedMessage = data.getBoolean("price_reduced")?"ÖS":"ÖV";
	...
	public void testGetNumberOut() {
		String expectedNumber = "0762778000";
		for(int i=0;i<okTickets.length;i++)
		{
			String ticket = okTickets[i];

...
public class VasterasTicketTest extends TestCase {
	...
	protected void setUp() throws Exception {
		...
		for (int i=0; i<okTickets.length; i++) {
			...
			hmTicket.put(ticket, new VasterasTicket());
	...
	public void testGetMessage() {
		...
		for(int i=0;i<okTickets.length;i++)
		{
			...
			expectedString = message[5].substring(message[5].length()-3) + " VL";
			assertEquals(expectedString,message[0]);
			
			assertEquals("", message[1]);
			
			expectedString = data.getBoolean("price_reduced")?"VS SKOLUNGDOM ":"VV VUXEN ";
			expectedString += "Giltig till " + validTime + " " + validDate;
			assertEquals(expectedString, message[2]);
			
			assertEquals("Västerås",message[3]);
	...
	public void testGetMessageOut() {
		...
		for(int i=0;i<okTickets.length;i++)
		{
			...
			expectedMessage = data.getBoolean("price_reduced")?"VS":"VV";
	...
	public void testGetNumberOut() {
		String expectedNumber = "0739304050";
		for(int i=0;i<okTickets.length;i++)
		{
			String ticket = okTickets[i];
